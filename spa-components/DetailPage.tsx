...
export const DetailPage: React.FC<DetailPageProps> = ({ result, onBack }) => {
  ...
  // 施設紹介文の状態管理
  const [description, setDescription] = useState<string | undefined>(result.description);
  ...
  useEffect(() => {
    ...
    fetchDescription();
  }, [result.placeId, description, result.title, result.address]);
        ...
        <div className="aspect-w-16 aspect-h-9" style={{ height: '400px' }}>
          {hasPhotos ? (
            <img
              ...
              alt={`${result.title} - ${currentPhotoIndex + 1}`}
              ...
              onError={(e) => {
                ...
                if (parent) {
                  ...
                  iframe.title = `Map of ${result.title}`;
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="border-t border-slate-200 pt-6">
          ...
          <ul className="space-y-4 text-slate-700">
                ...
                <div className="w-full">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {result.openingHours.open_now !== undefined && (
                        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                          result.openingHours.open_now
                            ? 'bg-green-100 text-green-800'
                            : 'bg-slate-100 text-slate-600'
                        }`}>
                          {result.openingHours.open_now ? '営業中' : '営業時間外'}
                        </span>
                      )}
                    </div>
                    <ChevronDownIcon
                  ...
                  {isOpeningHoursExpanded && (
                    <div className="mt-2">
                      {result.openingHours.weekday_text && result.openingHours.weekday_text.length > 0 ? (
                        <div className="space-y-1 text-sm">
                          {result.openingHours.weekday_text.map((dayText, index) => (
            ...
            {/* 営業状況 */}
            {result.businessStatus && getBusinessStatusInfo(result.businessStatus) && (
              <li className="flex items-start gap-4">
                {(() => {
                  const statusInfo = getBusinessStatusInfo(result.businessStatus);
            ...
            {/* 価格帯 */}
            {result.priceLevel !== undefined && getPriceRangeText(result.priceLevel) && (
              <li className="flex items-start gap-4">
                <YenIcon className="w-5 h-5 text-slate-400 mt-1 flex-shrink-0" />
                <div>
                  <span className="font-semibold block">目安価格帯</span>
                  <p className="text-slate-600 font-medium">{getPriceRangeText(result.priceLevel)}</p>
        ...
        <div className="border-t border-slate-200 pt-6">
          ...
          {result.detailedReviews && result.detailedReviews.length > 0 ? (
            <div className="space-y-4">
              {result.detailedReviews.map((review, index) => (
        ...
        {/* オーナー情報 Section */}
        {result.ownerInfo && (result.ownerInfo.message || (result.ownerInfo.posts && result.ownerInfo.posts.length > 0)) && (
          <div className="border-t border-slate-200 pt-6">
            <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
            ...
            <div className="space-y-4">
              ...
              {result.ownerInfo.message && (
                <div className="bg-gradient-to-r from-sky-50 to-blue-50 p-4 rounded-lg border border-sky-200">
                  ...
                  <p className="text-slate-800 whitespace-pre-wrap">{result.ownerInfo.message}</p>
                </div>
              )}

              {/* オーナーからの投稿 */}
              {result.ownerInfo.posts && result.ownerInfo.posts.length > 0 && (
                <div>
                  <p className="text-sm font-semibold text-slate-700 mb-2">オーナーからの投稿</p>
                  <div className="space-y-3">
                    {result.ownerInfo.posts.map((post, index) => (
